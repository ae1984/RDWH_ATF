create or replace package body dwh_buffer.ETL_20151202 is
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      execute immediate ('truncate table COL_T_PROCESS');
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      execute immediate ('truncate table COL_T_PROCMEM');
...
procedure FILL_COL_C_RNDMETH is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_C_RNDMETH');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('COL_C_RNDMETH_TMP');
      insert /*+ append */ into COL_C_RNDMETH_TMP 
      select 
           t.id      
          ,t.longname
          ,t.ndec    
          ,t.hilow   
          ,t.code    
      from colvir.C_RNDMETH@reps t;
      commit;  
      
      merge /*+ parallel(4) append */
      into COL_C_RNDMETH t
      using (select tt.* from COL_C_RNDMETH_TMP tt) a
      on (     t.id             = a.id          
           and t.longname       = a.longname   
           and t.ndec           = a.ndec       
           and t.hilow          = a.hilow      
           and t.code           = a.code       
         )
...
procedure FILL_COL_C_DOMAIN is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_C_DOMAIN');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('COL_C_DOMAIN_TMP');
      insert /*+ append */ into COL_C_DOMAIN_TMP 
      select 
           t.brn_id     
          ,t.code     
          ,t.dat      
          ...
          ,t.refsend  
          ,t.dchrused 
      from colvir.C_DOMAIN@reps t;
      commit;  
      
      merge /*+ parallel(4) append */
      into COL_C_DOMAIN t
      using (select tt.* from COL_C_DOMAIN_TMP tt) a
      on (     t.brn_id = a.brn_id    
           and t.code         = a.code     
           and t.dat          = a.dat      
           and t.p_code       = a.p_code   
           and t.dlen         = a.dlen     
           and t.dprec        = a.dprec    
           and t.predeffl     = a.predeffl 
           and t.defvalue     = a.defvalue 
           and t.editmask     = a.editmask 
           and t.clientfl     = a.clientfl 
           and t.validcl      = a.validcl  
           and t.ref_id       = a.ref_id   
           and t.delphityp    = a.delphityp
           and t.reffield     = a.reffield 
           and t.refform      = a.refform  
           and t.refsend      = a.refsend  
           and t.dchrused     = a.dchrused 
         )
      /*when matched then update set
            t.brn_id        = a.brn_id*/
      when not matched then insert
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('COL_CV_TBLROW_TMP');
      insert /*+ append */ into COL_CV_TBLROW_TMP 
      select 
           t.table_code
          ,t.table_name
          ,t.id        
      ...
      merge /*+ parallel(4) append */
      into COL_CV_TBLROW t
      using (select tt.* from COL_CV_TBLROW_TMP tt) a
      on (     t.table_code    = a.table_code 
           and t.table_name    = a.table_name
           and t.id            = a.id        
           and t.nord          = a.nord      
           and t.npp           = a.npp       
           and t.longname      = a.longname  
           and t.solution      = a.solution  
           and t.autoname      = a.autoname  
...
procedure FILL_COL_G_CON is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_G_CON');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('COL_G_CON_TMP');
      insert /*+ append */ into COL_G_CON_TMP 
      select 
           t.id        
          ,t.id_hi     
          ,t.code      
          ...
          ,t.anlround  
          ,t.ielevel   
      from colvir.G_CON@reps t;
      commit;  
      
      merge /*+ parallel(4) append */
      into COL_G_CON t
      using (select tt.* from COL_G_CON_TMP tt) a
      on (     t.id           = a.id        
           and t.id_hi        = a.id_hi     
           and t.code         = a.code      
           and t.groupfl      = a.groupfl   
           and t.nlevel       = a.nlevel    
           and t.arcfl        = a.arcfl     
           and t.arestfl      = a.arestfl   
           and t.activfl      = a.activfl   
           and t.id_us        = a.id_us     
           and t.correctdt    = a.correctdt 
           and t.longname     = a.longname  
           and t.prim         = a.prim      
           and t.multifl      = a.multifl   
           and t.act_knd      = a.act_knd   
           and t.per_id       = a.per_id    
           and t.nround       = a.nround    
           and t.varround     = a.varround  
           and t.formula      = a.formula   
           and t.autofl       = a.autofl    
           and t.norder       = a.norder    
           and t.fullfl       = a.fullfl    
           and t.sendfl       = a.sendfl    
           and t.genstatus    = a.genstatus 
           and t.rnd_proc     = a.rnd_proc  
           and t.chi_id       = a.chi_id    
           and t.act_dtl      = a.act_dtl   
           and t.fixdt        = a.fixdt     
           and t.anlround     = a.anlround  
           and t.ielevel      = a.ielevel   
         )
      /*when matched then update set
            t.brn_id        = a.brn_id*/
      when not matched then insert
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('COL_T_ARLDSC_TMP');
      insert /*+ append */ into COL_T_ARLDSC_TMP 
      select 
           t.id                     
          ,t.longname     
          ,t.sbj_id       
      ...
      merge /*+ parallel(4) append */
      into COL_T_ARLDSC t
      using (select tt.* from COL_T_ARLDSC_TMP tt) a
      on (     t.id            = a.id          
           and t.longname      = a.longname    
           and t.sbj_id        = a.sbj_id      
           and t.basetype      = a.basetype    
           and t.dtlbop_id     = a.dtlbop_id   
           and t.correctdt     = a.correctdt   
           and t.id_us         = a.id_us       
           and t.prim          = a.prim        
           and t.code          = a.code        
           and t.incomfl       = a.incomfl     
           and t.anchartfl     = a.anchartfl   
           and t.anlcha_id     = a.anlcha_id   
           and t.cha_id        = a.cha_id      
           and t.deashdfl      = a.deashdfl    
           and t.loopfl        = a.loopfl      
           and t.npp           = a.npp         
           and t.nopartfl      = a.nopartfl    
           and t.nopayfl       = a.nopayfl     
           and t.priority      = a.priority    
           and t.nonegfl       = a.nonegfl     
           and t.arestfl       = a.arestfl     
           and t.val_id        = a.val_id      
           and t.wroff_id      = a.wroff_id    
           and t.bnkatrusg     = a.bnkatrusg   
           and t.alias         = a.alias       
           and t.name_rpt      = a.name_rpt    
           and t.priority2     = a.priority2   
           and t.addfl         = a.addfl       
           and t.immediatefl   = a.immediatefl 
           and t.updshdfl      = a.updshdfl    
           and t.prepay        = a.prepay      
           and t.clcfl         = a.clcfl       
           and t.pay_priority  = a.pay_priority
           and t.alias_add     = a.alias_add   
         )
      /*when matched then update set
            t.brn_id        = a.brn_id*/
      when not matched then insert
...
procedure FILL_COL_T_DEAPRMDSC is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEAPRMDSC');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('COL_T_DEAPRMDSC_TMP');
      insert /*+ append */ into COL_T_DEAPRMDSC_TMP 
      select 
           t.id                     
          ,t.code         
          ,t.longname     
          ...
          ,t.mmfl         
          ,t.hideemptyfl  
      from colvir.T_DEAPRMDSC@reps t;
      commit;  
      
      merge /*+ parallel(4) append */
      into COL_T_DEAPRMDSC t
      using (select tt.* from COL_T_DEAPRMDSC_TMP tt) a
      on (     t.id           = a.id          
           and t.code         = a.code        
           and t.longname     = a.longname    
           and t.measunit     = a.measunit    
           and t.arcfl        = a.arcfl       
           and t.arestfl      = a.arestfl     
           and t.id_us        = a.id_us       
           and t.correctdt    = a.correctdt   
           and t.prim         = a.prim        
           and t.d_code       = a.d_code      
           and t.id_hi        = a.id_hi       
           and t.deafl        = a.deafl       
           and t.groupfl      = a.groupfl     
           and t.nlevel       = a.nlevel      
           and t.sbjcpt_typ   = a.sbjcpt_typ  
           and t.mmfl         = a.mmfl        
           and t.hideemptyfl  = a.hideemptyfl 
         )
      /*when matched then update set
            t.brn_id        = a.brn_id*/
      when not matched then insert
...
procedure FILL_COL_T_DEAPRMDSC_LNG is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEAPRMDSC_LNG');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('COL_T_DEAPRMDSC_LNG_TMP');
      insert /*+ append */ into COL_T_DEAPRMDSC_LNG_TMP 
      select 
           t.lng_id             
          ,t.id       
          ,t.longname 
          ,t.prim     
      from colvir.T_DEAPRMDSC_LNG@reps t;
      commit;  
      
      merge /*+ parallel(4) append */
      into COL_T_DEAPRMDSC_LNG t
      using (select tt.* from COL_T_DEAPRMDSC_LNG_TMP tt) a
      on (     t.lng_id      = a.lng_id      
           and t.id          = a.id          
           and t.longname    = a.longname    
           and t.prim        = a.prim        
         )
...
BEGIN
  ...
  insert /*+ append*/ into ETL_DM_LOAN_BASE 
  select /*+ parallel(32) */ /*-- ORDERED parallel(l,16) push_subq USE_NL(l,d, o, m,p,s,dp,dc, fil, qd) */
     d.id         ID,
     d.dep_id     DEP_ID,
     d.cli_id     CLI_ID,
     ...
     d.refer REFER_WH,
     qd.rate,
     max(case when acc.code = 'КФ01_О' then acc.acc_id else null  end) acc_id,
     max(case when acc.code = 'КФ01_О' then acc.accdep_id else null end) acc_dep_id,
     max(case when acc.code = 'КФ04_ОД' then acc.acc_id else null end) acc_pr_id,
     max(case when acc.code = 'КФ04_ОД' then acc.accdep_id else null end) acc_pr_dep_id,
     d.sell_dep_id as sell_dep_id,
     nvl(to_date(COL_T_PKGDEAPRM.fDeaParByCode(d.ID, d.DEP_ID, 'CSLN_FROMDATE')),d.FROMDATE) CSLN_FROMDATE,
     qd.id as OBJ_ID
  from col_l_dea l
    inner join col_t_dea d on d.dep_id = l.dep_id and d.id = l.id 
    left join col_T_ORD o  on o.DEP_ID = l.DEP_ID  and l.ID = o.ID
    left join col_T_VAL_STD v  on v.id = nvl(o.val_id, 1)
    left join col_T_PROCMEM m  on d.DEP_ID = m.DEP_ID and d.ID = m.ORD_ID and m.MAINFL = '1'
    left join col_T_PROCESS p on p.ID = m.ID
    left join col_T_BOP_STAT_STD s on s.ID = p.BOP_ID and s.NORD = p.NSTAT
    left join col_T_DEAPRD_std dp on dp.id = l.prd_id
    left join col_T_DEACLS_std dc on dc.id = d.dcl_id
    left join col_c_dep fil  on fil.id = d.dep_id
    left join col_q_dea qd on qd.refer = d.refer
    left join (select qa.obj_id, qa.acc_id, qa.accdep_id, typ.code
               ...
               where typ.id = qa.entacc_id
                     ...
                     and sysdate between qa.fromdate and qa.todate) acc on acc.obj_id = qd.id
  group by  d.id,
            d.dep_id,
            d.cli_id,
            d.cli_dep_id,
