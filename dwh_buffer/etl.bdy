create or replace package body dwh_buffer.ETL is
...
is
 CURSOR trans_buf_cur IS
  ...
  FROM v_trans o --view построенна на запросах в w4
  where  1=1
         and  o.AMND_DATE >= trunc(sysdate,'dd')-5 --информация за последние 5 дней
...
BEGIN
  ...
  execute immediate ('truncate table TRANS_BUFF_2'); --быстрая очистка временной таблицы
  ...
  loop
    ...
    insert /*+ append */
    into TRANS_BUFF_2 --заполение временной таблицы за последние 5 дней
    values
      (
  ...
  For rec in (select p.id_d,p.dir from TRANS_BUFF_2 p
      ...
      delete from TRANSACTION_3 t where t.id_d=rec.id_d and t.dir=rec.dir; --удаление повторяющихся записей из TRANSACTION_3, т.к. за 5 дней есть повторы
  ...
  insert /*+ parallel(16) append*/
  into "TRANSACTION_3" --инкрементное заполнение таблицы TRANSACTION_3
  select /*--append */
  ...
  from TRANS_BUFF_2 o
    left join OWS.MESS_CHANNEL@rpt t1 on o.source_channel = t1.code --таблица в w4 через dblink
    left join OWS.MESS_CHANNEL@rpt t2 on o.target_channel = t2.code --таблица в w4 через dblink
...
BEGIN
  ...
  execute immediate ('truncate table efee_list'); --очистка справочника компаний
  save_debug_log('Fill_PayRoll_List_New.  2.');
  insert /*-- parallel(bb,8) */
  into efee_list bb --заполнение справочника компаний
  select a.contract_number,
    ...
    from ows.ACNT_CONTRACT@rpt a  --таблица в w4 через dblink
   inner join ows.CONTR_STATUS@rpt c  --таблица в w4 через dblink
      on A.CONTR_STATUS = C.ID
   inner join ows.client@rpt b  --таблица в w4 через dblink
      on a.client__id = b.id
   inner join ows.SERV_PACK@rpt sp  --таблица в w4 через dblink
  ...
  execute immediate('truncate table sal_trans'); --очитска журнала зп транзакций
  save_debug_log('Fill_PayRoll_List_New.  4.');
  insert
  into sal_trans s --заполнение журнала зп транзакций с 1.1.2013года
  select  /*+ parallel (tr,2) parallel (cm,2) parallel (cl,2) parallel (ef,2) */
  coalesce(cl.iin,
   case
     when length(regexp_replace(b.itn, '[^0-9]', '')) = 12 then
      regexp_replace(b.itn, '[^0-9]', '')
   end) as iin_cl, --можно использовать просто select itn from ows.client@rpt, т.е. просто b.itn
   tr.target_number,
   sp.code service_pack_code,
   sp.name service_pack_name,
   tr.posting_date,
   ...
   inner join ows.ACNT_CONTRACT@rpt cont  --таблица в w4 через dblink
      on tr.target_number = cont.contract_number
     and cont.amnd_state = 'A'
   inner join OWS.OPT_SCHEME_N@rpt o  --таблица в w4 через dblink
   ...
   inner join ows.client@rpt b  --таблица в w4 через dblink
     ...
     and b.amnd_state = 'A'
   left join client_map cm on cm.client_src=b.id and cm.src='W' --для определения ИИН мы использовали данные из Colvir,
   left join client cl on cl.agr_id=cm.agr_id  -- можно использовать "select itn from ows.client@rpt"
   inner join ows.SERV_PACK@rpt sp  --таблица в w4 через dblink
   ...
   ---------LS 06112014--------------------
   delete from payroll_list_tmptt; --очистка временного справочника зарплатников
   commit;
   save_debug_log('Fill_PayRoll_List_New.  6.');
  insert into payroll_list_tmptt --заполнение временного справочника зарплатников
   /* select cast(a.iin_cl as varchar2(12)) iin,
    ...
    save_debug_log('Fill_PayRoll_List_New.  7.');
     ...
     if  Check_Etl_Tables('payroll_list_tmptt') then
        ...
        DBMS_MVIEW.REFRESH(LIST => 'payroll_list', METHOD => 'C'); --обновление справочника зарплатников 
  ...
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      loop
        ...
        commit;
        insert into CashAndGoForRB  select distinct t.* from CashAndGoForRB_tmp t where t.sdt = dt;
        commit;
      end loop;
      commit;
      close cur1;
...
procedure FILL_VCC_20160125 is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_VCC_20160125');
  l_action  etl_actions_log.id%type;
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      Truncate_Table('etl_CS_CLI_RM_VCC_20160125');
      insert into /*+append*/ etl_CS_CLI_RM_VCC_20160125 
      select /*+ parallel(16) */
       ID,DEP_ID,max(VCC) VCC, max(RMCODE) RMCODE
        from
        (select /*+ parallel(t,16) */ DISTINCT
              ID,DEP_ID,
                FIRST_VALUE(case when attr='VCC'
                                  then value
                                  end) OVER (PARTITION BY ID,DEP_ID,attr ORDER BY fromdate DESC) VCC,
                FIRST_VALUE(case when attr='RMCODE'
                                  then value
