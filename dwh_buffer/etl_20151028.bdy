create or replace package body dwh_buffer.ETL_20151028 is
gc_bulk_limit constant integer:=300000;
...
begin
  cSQL_command := 'select count(*) from '||p_tbl_name;
...
BEGIN
  ...
  execute immediate ('truncate table dm_cif_addr');
    ...
    b.APT F_APT,
    b.APT_T F_APT_T
    from (select * from etl_dm_cif_addr_tmp where addr_type='002') a --адрес регистрации
    full outer join (select * from etl_dm_cif_addr_tmp where addr_type='003') b -- фактический
    on a.id=b.id
...
BEGIN
  ...
  insert /*+ append */ /*-- parallel(a,8) Append */
  into DM_CIF_PHONE_NORMAL a
    select distinct coalesce(a.cli_id, c.cli_id) cli_id,
                    ...
                    a.dom,
                    c.mob
      from (select distinct coalesce(a.cli_id, b.cli_id) cli_id,
                            coalesce(a.dep_id, b.dep_id) dep_id,
                            coalesce(a.cli_code, b.cli_code) cli_code,
                            a.rab,
                            b.dom
              from (select c.id cli_id,
                           c.dep_id,
                           c.code CLI_CODE,
                           LISTAGG(t.pnum, ', ') WITHIN GROUP(order by nvl(t.basfl, 0), nvl(correctdt, to_date('01.01.1900', 'dd.mm.yyyy'))) rab
                      from col_G_CLIPHONE t, col_g_cli c
                     where c.id = t.id
                     ...
                     group by c.id, c.dep_id, c.code) a
              full outer join (select c.id cli_id,
                                     c.dep_id,
                                     c.code CLI_CODE,
                               ...
                               where c.id = t.id
                                 ...
                                 and t.pnum is not null
                               group by c.id, c.dep_id, c.code) b
                on a.cli_id = b.cli_id
               and a.dep_id = b.dep_id
               and a.cli_code = b.cli_code) a
      full outer join (select c.id cli_id,
                              c.dep_id,
                              c.code CLI_CODE,
                        ...
                        where c.id = t.id
                          ...
                          and t.pnum is not null
                        group by c.id, c.dep_id, c.code) c
...
BEGIN
  ...
  insert /*+ parallel(16) append*/ /*-- parallel(b,8) Append*/
  ...
  select count(*) into i from DM_BAL_SNAP_TMP;
  if i>0 then
...
BEGIN
  ...
  using (select d.prc,
         ...
         --from colvir.etl_dep_base_2@reps d
           from etl_dep_base_2 d) a
  on (t.ID = a.id and t.dep_id = a.dep_id)
  when matched then
    update
       set t.CLC_ID         = a.CLC_ID,
           t.PRC_ID         = a.PRC_ID,
           t.BOP_ID         = a.BOP_ID,
           t.CLI_ID         = a.CLI_ID,
...
BEGIN
    ...
    save_debug_log('Fill_W4_Contract.  3.');
      ...
      insert into ows_contract_changes
      select /*+append*/
      a.amnd_prev ows_contract_id,
      a.amnd_date,
      nvl(LEAD(a.amnd_date,1) over (partition by a.amnd_prev order by a.amnd_date),to_date('31.12.2999')) todate,
      a.amnd_officer,
      a.id,
      ...
      insert into ows_client_changes
      select /*+append*/
      ...
      a.is_ready,
      case when add_info_04 like '%<VCC>%'
      and regexp_like(substr(add_info_04,6,6),'[0-9]+')
      then substr(add_info_04,6,6) end vcc
...
is
 CURSOR trans_buf_cur IS
  ...
  FROM v_trans o
  where  1=1
         and  o.AMND_DATE >= trunc(sysdate,'dd')-7
...
BEGIN
  ...
  open trans_buf_cur;
  loop
    fetch trans_buf_cur bulk collect into l_trans_buf_recs limit gc_bulk_limit;
    exit when l_trans_buf_recs.count=0;

    forall i in indices of l_trans_buf_recs
    insert into TRANS_BUFF_2
    values
      (
  ...
  commit;
  save_debug_log('Fill_Trans_new.  4.');
...
BEGIN
 ...
 select l.id          contract_id,
         ...
         acc.acc_code,
         anlcode
 from col_l_dea l --Договор кредита
   inner join col_t_dea d  on d.id =l.id and d.dep_id = l.dep_id --Сделки и договора
   inner join col_q_dea qd  on qd.refer = d.refer --Cделки
   inner join (select qa.obj_id,
                      qa.acc_id,
                      qa.accdep_id,
                      typ.code,
                      typ.longname,
                      qa.code acc_code,
                      typ.anlcode
                from col_q_acc qa, col_Q_ENTACC_std typ --Счета ARM, Типы счетов сущностей
                where typ.id = qa.entacc_id and sysdate between qa.fromdate and qa.todate) acc  on acc.obj_id = qd.id
 where acc.code <> 'CR_EXP_IN'; --CR_EXP_IN Просрочено проценты
 commit;
 truncate_table('etl_dm_loan_acc_OVD');
 insert /*+ parallel(16) append */
 into etl_dm_loan_acc_OVD
 select * from DM_LOAN_ACC t
 where acc_type_code in ('КФ04_ОД', 'CR_WR_PD', 'КФ01_О');
 commit;
 save_debug_log('Fill_LOYAL_New.  2.');
  
 delete from dm_loan_acc a
 where (acc_id, acc_type_code) in
         (select /*+ parallel(16) */ acc_id, max(acc_type_code) acc_type_code
          from dm_loan_acc
          where ACC_TYPE_CODE not in ('КФ04_П', 'CR_EXP_IN')
                 /*
                  CR_EXP_IN	Просрочено проценты
                  КФ04_П	Просроченные проценты по кредиту
                 */
          group by acc_id
          having count(*) > 1);
   commit;
   save_debug_log('Fill_LOYAL_New.  3.');
   /*
    КФ04_ОД	Просроченный основной долг
    CR_WR_PD	Списано основной долг
    КФ01_О	Остаток долга
    CR_WR_PD	Списанный основной долг
   */
   ...
   select /*+ parallel(t,16)*/
       /*--use_hash(t,a) parallel(t,16)*/
   t.*,a.anlcode
   from col_t_bal t
   inner join etl_dm_loan_acc_ovd a on t.id=a.acc_id
   ;
   commit;
   save_debug_log('Fill_LOYAL_New.  6.');
   ...
   from (
        ...
        from bal_loan_ovd a
          left join bal_loan_ovd b on a.id = b.id and a.val_id = b.val_id and b.flzo = 1 and b.fromdate <= a.fromdate
        where a.flzo = 0 and a.anlcode in ('CR_EXP_PD', 'CR_WR_PD')
   ) a;
   ...
   save_debug_log('Fill_LOYAL_New.  9.');
   ...
   from dm_loan_BAL_OVD a
     inner join dm_loan_acc acc  on a.ACC_ID = acc.ACC_ID  and acc.ACC_TYPE_CODE = 'КФ04_ОД';

   commit;
   save_debug_log('Fill_LOYAL_New.  11.');
   ...
   select 
           ...
           a.natval_OUT + a.natval_out_FLZO natval_out
   from dm_loan_BAL_OVD a
     inner join dm_loan_acc acc on a.ACC_ID = acc.ACC_ID and acc.ACC_TYPE_CODE = 'CR_WR_PD'
   ;

   commit;
    save_debug_log('Fill_LOYAL_New.  13.');
    ...
    from (select a.val_id, acc.CONTRACT_ID, a.FROMDATE
     ...
     commit;
     save_debug_log('Fill_LOYAL_New.  15.');
     ...
     */
     ...
     ;
      ...
      commit;
      save_debug_log('Fill_LOYAL_New.  18.');
     ...
     commit;
     save_debug_log('Fill_LOYAL_New.  20.');
    ...
    save_debug_log('Fill_LOYAL_New.  23.');
    ...
    commit;
    save_debug_log('Fill_LOYAL_New.  25.');
    ...
    */
    ...
    commit;
    save_debug_log('Fill_LOYAL_New.  27.');
    ...
    commit;
    save_debug_log('Fill_LOYAL_New.  29.');
    ...
    distinct a.val_id,
     ...
     commit;
     save_debug_log('Fill_LOYAL_New.  31.');
     ...
     into loan_bal_tmp_5
      ...
      distinct a.val_id,
       ...
       commit;
       save_debug_log('Fill_LOYAL_New.  33.');
       ...
       select 
        ...
        commit;
        save_debug_log('Fill_LOYAL_New.  35.');
   ...
   commit;
   save_debug_log('Fill_LOYAL_New.  37.');
   ...
   into LOYAL_FIN_TMPTT
        ...
        left join (select distinct b.cli_id
                    ...
                    inner join loyal_1 b
                      ...
                      and add_months(case
                                     ...
                                     -6) > dt_end
                      and add_months(trunc(sysdate) - 1, -6) > dt_end
                      and term > 30) d
     ...
     ;
     commit;
     save_debug_log('Fill_LOYAL_New.  39.');
     ...
     if  Check_Etl_Tables('loyal_fin_tmptt') then
         -- commit; ---'ok, data can be inserted'
          ...
          insert into loyal_fin
          select * from LOYAL_FIN_TMPTT;
          commit;
          save_debug_log('Fill_LOYAL_New.  42.');
      else
          rollback;
       --   gv_err_count:=gv_err_count+1;
...
into contr_list_risk
  select dl.contract_id,
         ...
         case
           when target_code_atf = '14' or
                lower(product_name_atf) like '%ипотека%' then
            '1. Mortgage'
           when product_code_atf in
                ('4.4', '1.13', '1.14', '1.15', '1.16', '1.17') then
            '2. Auto'
           when product_code_atf in ('1.12',
                                     '4.3',
                                     '1.29',
                                     '1.30',
                                     '1.31',
         ...
         der.effective_rate wt_effective_rate,
         
         case
           when lower(dep.longname_hi) like '%филиал%' then
            dep.longname_hi
           when lower(dep.longname) like '%филиал%' then
            dep.longname
         end filial_name,
         dep.LONGNAME branch_name,
         max(case
               when acc_type_code = 'КФ01_О' then
                dlc.acc_id
             end) principal_acc_id,
         max(case
               when acc_type_code = 'КФ04_ОД' then
                dlc.acc_id
             end) ovd_principal_acc_id,
         max(case
               when acc_type_code = 'CR_WR_PD' then
                dlc.acc_id
             end) OW_principal_acc_id,
         case
           when dcb.vcc in
                ('107320', '107329', '107330', '107340', '107390', '999992') then
            '1.Retail'
           when dcb.vcc in ('107310', '107319', '999994') then
            '2.Private'
           when dcb.vcc in
                ('104120', '104810', '104815', '104819', '999993') then
            '3.SME'
           else
            '4. Corporate'
         end cl_type,
   ...
   inner join DM_LOAN_BASE_add da
    ...
    left join DICT_EFFECTIVE_RATE der
      on der.term =
         least(greatest(round(decode(term_type,
                                     null,
                                     months_between(todate, fromdate),
                                     term_cnt / decode(term_type,
                                                       'M',
                                                       1,
                                                       'D',
                                                       30.4167,
               ...
               180)
     and der.rate =
         least(greatest(round(coalesce(dl.rate, pcn_rate, 0) * 2) / 2, 0),
               50)
     and der.COMM =
         least(greatest(round(coalesce(pcn_com, 0) * 2) / 2, 0), 20)
...
into risk_cube
  select dl.*,
         ...
         d.AMOUNT_VAL * etl.Get_NBRK_Curr(trunc(sysdate), '', dl.val_id) -
         d.paysdok * etl.Get_NBRK_Curr(trunc(sysdate), '', dl.val_id) payment_amount_fact,
         ROW_NUMBER() OVER(PARTITION BY d.contract_id, d.LONGNAME ORDER BY d.doper) payment_number,
         case
           when trunc(sysdate) - 1 - d.doper >= 1 then
            1
           else
            null
         end censored_01,
         case
           when trunc(sysdate) - 1 - d.doper >= 7 then
            1
           else
            null
         end censored_07,
         case
           when trunc(sysdate) - 1 - d.doper >= 14 then
            1
           else
            null
         end censored_14,
         case
           when trunc(sysdate) - 1 - d.doper >= 21 then
            1
           else
            null
         end censored_21,
         case
           when trunc(sysdate) - 1 - d.doper >= 30 then
            1
           else
            null
         end censored_30,
         case
           when trunc(sysdate) - 1 - d.doper >= 60 then
            1
           else
            null
         end censored_60,
         case
           when trunc(sysdate) - 1 - d.doper >= 90 then
            1
           else
            null
         end censored_90,
         ...
         (case
           when nvl(d.DCLOSE, trunc(sysdate) - 1) - d.doper >= 1 then
            1
           else
            0
         end) * (case
         ...
         (case
           when nvl(d.DCLOSE, trunc(sysdate) - 1) - d.doper >= 7 then
            1
           else
            0
         end) * (case
         ...
         (case
           when nvl(d.DCLOSE, trunc(sysdate) - 1) - d.doper >= 14 then
            1
           else
            0
         end) * (case
         ...
         (case
           when nvl(d.DCLOSE, trunc(sysdate) - 1) - d.doper >= 21 then
            1
           else
            0
         end) * (case
         ...
         (case
           when nvl(d.DCLOSE, trunc(sysdate) - 1) - d.doper >= 30 then
            1
           else
            0
         end) * (case
         ...
         (case
           when nvl(d.DCLOSE, trunc(sysdate) - 1) - d.doper >= 60 then
            1
           else
            0
         end) * (case
         ...
         (case
           when nvl(d.DCLOSE, trunc(sysdate) - 1) - d.doper >= 90 then
            1
           else
            0
         end) * (case
   ...
   where 1 = 1
     and d.LONGNAME in
         ('Проценты по кредиту',
          'Основной долг',
          'Комиссия за  обслуживания банковского займа')
...
BEGIN
 if check_sysdate_state (l_act_name) is null then
  BEGIN
    ...
    --colvir.ETL_DWH.run_thread_1@reps;  перенес в DWH 26.10.2015
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      insert /*+ append */ into NBRK_CURRENCY_RATE 
      select 
          t.Fromdate as DT
          ,t.Rate  as rate 
          ,a.CODE as alph_code
      ...
      loop
         ...
         if cnt <=0 then
           ...
           select 
              ...
              ,t.natval_in 
              ,t.natval_out
           from colvir.T_BAL@reps t where t.FROMDATE = v_start_dt+i;
           commit;
         end if;
...
procedure FILL_COL_L_DEA is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_L_DEA');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_L_DEA t;
      insert /*+ append */ into COL_L_DEA 
      select 
         t.dep_id         
         ,t.id            
         ,t.prd_id        
...
procedure FILL_COL_T_DEA is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEA');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_DEA t;
      insert /*+ append */ into COL_T_DEA 
      select 
           t.dep_id        
          ,t.id            
          ,t.dcl_id        
...
procedure FILL_COL_Q_DEA is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_Q_DEA');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_Q_DEA t;
      insert /*+ append */ into COL_Q_DEA 
      select 
           t.id       
          ,t.dep_id   
          ,t.type_id  
...
procedure FILL_COL_Q_ENTACC_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_Q_ENTACC_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_Q_ENTACC_STD t;
      insert /*+ append */ into COL_Q_ENTACC_STD 
      select 
           t.id      
          ,t.code    
          ,t.longname
          ...
          ,t.arcfl   
          ,t.npp     
      from colvir.Q_ENTACC_STD@reps t
      where t.id > v_id;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_Q_ACC is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_Q_ACC');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_Q_ACC t;
      execute immediate ('truncate table COL_Q_ACC');
      insert /*+ append */ into COL_Q_ACC 
      select 
           t.obj_id   
          ,t.entacc_id
          ,t.code     
...
procedure FILL_COL_T_DEAPRM is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEAPRM');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      execute immediate ('truncate table COL_T_DEAPRM');
      insert /*+ append */ into COL_T_DEAPRM 
      select 
          t.dep_id    
         ,t.id      
         ,t.dea_id  
         ,t.parvalue
      from colvir.T_DEAPRM@reps t 
      --left join COL_T_DEAPRM a on a.DEP_ID = t.DEP_ID and a.ID = t.ID and a.DEA_ID = t.DEA_ID
      --where a.id is null;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_DEAPRMDSC_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEAPRMDSC_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_DEAPRMDSC_STD t;
      insert /*+ append */ into COL_T_DEAPRMDSC_STD 
      select 
            t.id         
           ,t.code       
           ,t.longname   
           ...
           ,t.mmfl       
           ,t.hideemptyfl
      from colvir.T_DEAPRMDSC_STD@reps t
      where t.id > v_id;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_VAL_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_VAL_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_VAL_STD t;
      insert /*+ append */ into COL_T_VAL_STD 
      select  
          t.id         
         ,t.code       
         ,t.arcfl      
...
procedure FILL_COL_T_ORD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_ORD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_ORD t;
      insert /*+ append */ into COL_T_ORD 
      select 
           t.dep_id   
          ,t.id       
          ,t.ord_id   
...
procedure FILL_COL_T_PROCESS is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_PROCESS');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_PROCESS t;
      insert /*+ append */ into COL_T_PROCESS 
      select 
          t.id        
         ,t.bop_id    
         ,t.nstat     
...
procedure FILL_COL_T_BOP_STAT_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_BOP_STAT_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_T_BOP_STAT_STD');
      insert /*+ append */ into COL_T_BOP_STAT_STD 
      select 
           t.id          
          ,t.nord        
          ,t.code        
          ...
          ,t.color       
          ,t.brn_interact
      from colvir.T_BOP_STAT_STD@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_DEAPRD_std is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEAPRD_std');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_DEAPRD_std t;
      insert /*+ append */ into COL_T_DEAPRD_std 
      select 
           t.id       
          ,t.longname 
          ,t.time_type
          ...
          ,t.prim     
          ,t.code     
      from colvir.T_DEAPRD_std@reps t
      where t.id > v_id;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_DEACLS_std is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEACLS_std');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_DEACLS_std t;
      insert /*+ append */ into COL_T_DEACLS_std
      select 
           t.id       
          ,t.id_hi    
          ,t.code     
          ...
          ,t.sbj_id   
          ,t.ddd_id   
      from colvir.T_DEACLS_std@reps t
      where t.id > v_id;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_c_dep is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_c_dep');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_c_dep t;
      insert /*+ append */ into COL_c_dep
      select 
           t.id       
          ,t.id_hi    
          ,t.code     
...
procedure FILL_COL_G_CLIADDATR is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_G_CLIADDATR');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_G_CLIADDATR');
      insert /*+ append */ into COL_G_CLIADDATR 
      select 
           t.dep_id  
          ,t.id      
          ,t.fromdate
          ,t.attr    
          ,t.value   
      from colvir.G_CLIADDATR@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_PROCMEM is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_PROCMEM');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_PROCMEM t;
      insert /*+ append */  into COL_T_PROCMEM
      select 
          t.id     
         ,t.bop_id 
         ,t.nmem   
...
procedure FILL_COL_T_DEASHDPNT is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEASHDPNT');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_DEASHDPNT t;
      execute immediate ('truncate table COL_T_DEASHDPNT');
      insert /*+ append */ into  COL_T_DEASHDPNT
      select 
            t.id        
           ,t.dep_id    
           ,t.ord_id    
           ...
           ,t.doper     
           ,t.waitdate  
      from colvir.T_DEASHDPNT@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_DEAPAY is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_DEAPAY');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      gv_start:=dbms_utility.get_time;
      Store_Etl_Action(l_act_name,l_action);
      select max(id) into v_id from COL_T_DEAPAY t;
      insert /*+ append */ into  COL_T_DEAPAY
      select 
           t.dep_id      
           ,t.id          
           ,t.dea_id      
...
procedure FILL_COL_T_ARLDEA is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_ARLDEA');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_DEASHDPNT t;
      execute immediate ('truncate table COL_T_ARLDEA');
      insert /*+ append */ into  COL_T_ARLDEA
      select 
            t.dep_id    
           ,t.clc_id    
           ,t.ord_id    
...
procedure FILL_COL_T_ARLCLC is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_ARLCLC');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_DEASHDPNT t;
      execute immediate ('truncate table COL_T_ARLCLC');
      insert /*+ append */ into  COL_T_ARLCLC
      select 
            t.id          
           ,t.arl_id      
           ,t.shd_id      
...
procedure FILL_COL_T_ARLDSC is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_ARLDSC');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_DEASHDPNT t;
      execute immediate ('truncate table COL_T_ARLDSC');
      insert /*+ append */ into  COL_T_ARLDSC
      select 
            t.id           
           ,t.longname     
           ,t.sbj_id       
...
procedure FILL_COL_g_cli is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_g_cli');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_g_cli');
      insert /*+ append */ into COL_g_cli 
      select 
           t.dep_id     
          ,t.id         
          ,t.code       
...
procedure FILL_COL_g_cliaddress is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_g_cliaddress');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_g_cliaddress');
      insert /*+ append */ into COL_g_cliaddress 
      select 
           t.dep_id   
          ,t.id       
          ,t.nord     
          ...
          ,t.correctdt
          ,t.adr_id   
      from colvir.g_cliaddress@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_g_address is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_g_address');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_g_address');
      insert /*+ append */ into COL_g_address 
      select 
           t.id                  
          ,t.id_hi      
          ,t.nlevel     
...
procedure FILL_COL_G_TERRDSC is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_G_TERRDSC');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_G_TERRDSC');
      insert /*+ append */ into COL_G_TERRDSC
      select 
           t.id                 
          ,t.longname  
          ,t.shortname 
...
procedure FILL_COL_LEDACC_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_LEDACC_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_LEDACC_STD');
      insert /*+ append */ into COL_LEDACC_STD
      select 
           t.id                  
          ,t.code       
          ,t.part_code  
...
procedure FILL_COL_G_ACCBLNHST is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_G_ACCBLNHST');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_G_ACCBLNHST');
      insert /*+ append */ into COL_G_ACCBLNHST
      select 
           t.dep_id             
          ,t.id        
          ,t.fromdate  
          ...
          ,t.reldep_id 
          ,t.rel_id    
      from colvir.G_ACCBLNHST@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_G_ACCBLN is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_G_ACCBLN');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_G_ACCBLN');
      insert /*+ append */ into COL_G_ACCBLN
      select 
           t.dep_id           
          ,t.id        
          ,t.cha_id    
...
procedure FILL_COL_G_IDENTDOCDSC_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_G_IDENTDOCDSC_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_G_IDENTDOCDSC_STD');
      insert /*+ append */ into COL_G_IDENTDOCDSC_STD 
      select 
           t.id          
          ,t.code        
          ,t.name        
          ...
          ,t.numchk_dsc  
          ,t.def_proc    
      from colvir.G_IDENTDOCDSC_STD@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_REG_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_REG_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_T_REG_STD');
      insert /*+ append */ into COL_T_REG_STD 
      select 
           t.id                  
          ,t.id_hi      
          ,t.val_id     
...
procedure FILL_COL_g_Clihst is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_g_Clihst');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_g_Clihst');
      insert /*+ append */ into COL_g_Clihst 
      select 
           t.dep_id            
          ,t.id                
          ,t.fromdate          
...
procedure FILL_COL_L_MRTDEA is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_L_MRTDEA');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_L_MRTDEA');
      insert /*+ append */ into COL_L_MRTDEA 
      select 
           t.dea_dep_id        
          ,t.dea_id            
          ,t.mrt_dep_id        
...
procedure FILL_COL_L_MORTGAGE is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_L_MORTGAGE');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_L_MORTGAGE');
      insert /*+ append */ into COL_L_MORTGAGE 
      select 
           t.dep_id                      
          ,t.id               
          ,t.dscr             
...
procedure FILL_COL_L_ENSDSC_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_L_ENSDSC_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_L_ENSDSC_STD');
      insert /*+ append */ into COL_L_ENSDSC_STD 
      select 
           t.id                       
          ,t.id_hi           
          ,t.code            
          ...
          ,t.acomplyearfl    
          ,t.ddd_id          
      from colvir.L_ENSDSC_STD@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_G_CLIPHONE is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_G_CLIPHONE');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_G_CLIPHONE');
      insert /*+ append */ into COL_G_CLIPHONE 
      select 
           t.dep_id              
          ,t.id         
          ,t.nord       
...
procedure FILL_COL_T_ARLCLS is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_ARLCLS');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_T_ARLCLS');
      insert /*+ append */ into COL_T_ARLCLS 
      select 
           t.dcl_id              
          ,t.clc_id     
          ,t.deaeditfl  
...
procedure FILL_COL_T_PCN is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_PCN');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_T_PCN');
      insert /*+ append */ into COL_T_PCN 
      select 
           t.id                
          ,t.code     
          ,t.specfl   
          ,t.type_id  
      from colvir.T_PCN@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_ARLMETHPRM is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_ARLMETHPRM');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_T_ARLMETHPRM');
      insert /*+ append */ into COL_T_ARLMETHPRM 
      select 
           t.id                
          ,t.meth     
          ,t.par_code 
          ,t.value    
          ,t.arcfl    
      from colvir.T_ARLMETHPRM@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_T_PCNHST is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_t_pcnhst');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_t_pcnhst');
      insert /*+ append */ into COL_t_pcnhst 
      select 
           t.id           
          ,t.nver         
          ,t.fromdate     
...
procedure FILL_COL_T_PCNDSC is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_PCNDSC');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_T_PCNDSC');
      insert /*+ append */ into COL_T_PCNDSC 
      select 
           t.id        
          ,t.nver      
          ,t.vrf_id    
...
procedure FILL_COL_T_PCNDTL is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_T_PCNDTL');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_T_PCNDTL');
      insert /*+ append */ into COL_T_PCNDTL 
      select 
           t.id           
          ,t.nver         
          ,t.fromdate     
...
procedure FILL_COL_C_TBLCOL_STD is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_C_TBLCOL_STD');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_C_TBLCOL_STD');
      insert /*+ append */ into COL_C_TBLCOL_STD 
      select 
           t.id       
          ,t.nord     
          ,t.sbj_id   
          ...
          ,t.width    
          ,t.code     
      from colvir.C_TBLCOL_STD@reps t
      --where t.id > v_id;
      ;
      commit;     
      Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100);
...
procedure FILL_COL_CV_TBLROW is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_CV_TBLROW');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_CV_TBLROW');
      insert /*+ append */ into COL_CV_TBLROW 
      select 
           t.table_code 
          ,t.table_name 
          ,t.id         
...
procedure FILL_COL_C_TBLVAL is
  l_act_name constant etl_actions_log.action_name%type:=Upper('FILL_COL_C_TBLVAL');
  l_action  etl_actions_log.id%type;
...
begin
  if check_sysdate_state (l_act_name) is null then
    BEGIN
      ...
      Store_Etl_Action(l_act_name,l_action);
      --select max(id) into v_id from COL_T_PROCESS t;
      execute immediate ('truncate table COL_C_TBLVAL');
      insert /*+ append */ into COL_C_TBLVAL 
      select 
           t.id       
          ,t.nord     
          ,t.nord_col 
...
procedure Prepare_etl_CS_CLI_RM_VCC
  is
  l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_CS_CLI_RM_VCC');
  l_action  etl_actions_log.id%type;
...
BEGIN
  ...
  Truncate_Table('ETL_CS_CLI_RM_VCC');

  insert /*+ parallel(16) */
  into ETL_CS_CLI_RM_VCC
  select ID, DEP_ID, max(VCC) VCC, max(RMCODE) RMCODE
  from (select /*+ parallel(t,16) */
          DISTINCT ID,
                   ...
                   FIRST_VALUE(case
                                 when attr = 'VCC' then
                                  value
                               end) OVER(PARTITION BY ID, DEP_ID, attr ORDER BY fromdate DESC) VCC,
                   FIRST_VALUE(case
                                 when attr = 'RMCODE' then
                                  value
                               end) OVER(PARTITION BY ID, DEP_ID, attr ORDER BY fromdate DESC) RMCODE
  from COL_G_CLIADDATR t
  where attr in ('VCC', 'RMCODE'))
  group by ID, DEP_ID;
  commit;
  Update_Etl_action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100,null);
exception
  when others then
    rollback;
    gv_err_count:=gv_err_count+1;
...
procedure Prepare_etl_dm_loan_base
  is
l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_dm_loan_base');
l_action etl_actions_log.id%type;
begin
if check_sysdate_state (l_act_name) is null then
BEGIN
  ...
  Truncate_Table('etl_dm_loan_base');

  insert /*+ parallel(16) */
  into etl_dm_loan_base a
  select /*+ ORDERED parallel(l,16) push_subq USE_NL(l,d, o, m,p,s,dp,dc, fil, qd) */
   d.id         ID,
   d.dep_id     DEP_ID,
   d.cli_id     CLI_ID,
   d.cli_dep_id CLI_DEP_ID,
   o.code       CONTRACT_NUM,
   o.val_id     VAL_ID,
   v.code       VAL_CODE,
   dc.code      PRD_CODE,
   dc.longname  PRD_NAME,
   dp.longname  PRD_TERM,
   dp.time_type TERM_TYPE,
   dp.time_unit TERM_CNT,
   d.fromdate   FROMDATE,
   d.todate     TODATE,
   s.longname   STATE,
   fil.code     FIL_CODE,
   fil.longname FIL_NAME,
   d.sdok       sum_full, --qd.sum_full,
   --qd.refer REFER_WH,
   d.refer REFER_WH,
   qd.rate,
   max(case
         when acc.code = 'КФ01_О' then
          acc.acc_id
         else
          null
       end) acc_id,
   ...
   d.sell_dep_id as sell_dep_id,
   nvl(to_date(COL_T_PKGDEAPRM.fDeaParByCode(d.ID, d.DEP_ID, 'CSLN_FROMDATE')),
       d.FROMDATE) CSLN_FROMDATE,
   qd.id as OBJ_ID
  from col_l_dea l
   inner join col_t_dea d
      on l.id = d.id
     and d.dep_id = l.dep_id
    left join col_T_ORD o
      on o.DEP_ID = l.DEP_ID
     and l.ID = o.ID
    left join col_T_VAL_STD v
      on v.id = nvl(o.val_id, 1)
    left join col_T_PROCMEM m
      on d.DEP_ID = m.DEP_ID
     and d.ID = m.ORD_ID
     and m.MAINFL = '1'
    left join col_T_PROCESS p
      on p.ID = m.ID
    left join col_T_BOP_STAT_STD s
      on s.ID = p.BOP_ID
     and s.NORD = p.NSTAT
    left join col_T_DEAPRD_std dp
      on dp.id = l.prd_id
    left join col_T_DEACLS_std dc
      on dc.id = d.dcl_id
    left join col_c_dep fil
      on fil.id = d.dep_id
    left join col_q_dea qd
      on qd.refer = d.refer
    left join (select qa.obj_id, qa.acc_id, qa.accdep_id, typ.code
                 from col_q_acc qa, col_Q_ENTACC_std typ
                where typ.id = qa.entacc_id
                  and typ.code in ('КФ01_О', 'КФ04_ОД')
...
procedure Prepare_etl_buf
  is
 l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_buf');
 l_action etl_actions_log.id%type;
BEGIN
if check_sysdate_state (l_act_name) is null then
BEGIN
  ...
  truncate_table('etl_buf');

  insert /*+ parallel(16) Append */
  into etl_buf
    select l.id, l.dep_id, d.LONGNAME, c.arl_id, ad.clc_id, s.fromdate
      from col_l_dea l
     inner join col_T_ARLDEA ad
        on ad.DEP_ID = l.dep_id
       and ad.ORD_ID = l.ID
     inner join col_T_ARLCLC c
        on ad.CLC_ID = c.ID
     inner join col_T_ARLDSC d
        on c.ARL_ID = d.ID
     inner join col_t_dea s
        on l.id = s.id
       and s.dep_id = l.dep_id;
...
procedure Prepare_etl_dm_loan_paym_3
  is
 l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_dm_loan_paym_3');
 l_action etl_actions_log.id%type;
BEGIN
if check_sysdate_state (l_act_name) is null then
BEGIN
  ...
  --truncate_table('etl_dm_loan_paym_3');
  truncate_table('dm_loan_paym_3');
  insert /*+ parallel(16) Append */
  into dm_loan_paym_3 --etl_dm_loan_paym_3
  select /*+use_hash(s,b) parallel (s,4) parallel (b,4)*/
    b.id contract_id,
    s.ID,
    b.LONGNAME,
    s.DOPER,
    s.AMOUNT as AMOUNT_val,
    s.PRC,
    s.DCLCFROM,
    s.DCLCTO,
    T.CSR_CODE,
    ...
    T.SDOKn,
    T.FIX_PAY_NORD
  from col_T_DEASHDPNT s
  inner join etl_buf b
  on s.ORD_ID=b.ID
    and s.DEP_ID=b.DEP_ID
    and s.CLC_ID=b.CLC_ID
  left JOIN col_T_DEAPAY T
  ON S.ORD_ID=T.DEA_ID
    and s.dep_id=t.dep_id
    and s.tt_id=t.tt_id
    and s.tt_nord=t.tt_nord
    and s.clc_id=t.clc_id
  ;
  commit;
...
procedure Prepare_etl_dm_cif_addr_tmp
  is
 l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_dm_cif_addr_tmp');
 l_action etl_actions_log.id%type;
begin
if check_sysdate_state (l_act_name) is null then
BEGIN
  ...
  truncate_table('etl_dm_cif_addr_tmp');

  insert /*+ parallel(16) Append */
  into etl_dm_cif_addr_tmp
  select /*-- ORDERED parallel(g,16)  USE_NL(g, gg1, gg2,gg3,gg4,gg5,gg6,gg7,gg8,gg9) */
    (select code from col_g_cli where id=g.id and dep_id=g.dep_id) CLI_CODE,
    g.id , g.dep_id, g.adrtype ADDR_TYPE, gg1.post_index,
      case when nvl(gg1.addrpart,'01') in ('01') then gg1.longname
           when nvl(gg2.addrpart,'01') in ('01') then gg2.longname
           when nvl(gg3.addrpart,'01') in ('01') then gg3.longname
           when nvl(gg4.addrpart,'01') in ('01') then gg4.longname
           when nvl(gg5.addrpart,'01') in ('01') then gg5.longname
           when nvl(gg6.addrpart,'01') in ('01') then gg6.longname
           when nvl(gg7.addrpart,'01') in ('01') then gg7.longname
           when nvl(gg8.addrpart,'01') in ('01') then gg8.longname
           when nvl(gg9.addrpart,'01') in ('01') then gg9.longname
           else '#' end as STRANA,
      case when gg1.addrpart in ('02') then gg1.longname
           when gg2.addrpart in ('02') then gg2.longname
           when gg3.addrpart in ('02') then gg3.longname
           when gg4.addrpart in ('02') then gg4.longname
           when gg5.addrpart in ('02') then gg5.longname
           when gg6.addrpart in ('02') then gg6.longname
           when gg7.addrpart in ('02') then gg7.longname
           when gg8.addrpart in ('02') then gg8.longname
           when gg9.addrpart in ('02') then gg9.longname
           else '#' end as OBL,
      case when gg1.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('02') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as OBL_T,
      case when gg1.addrpart in ('03') then gg1.longname
           when gg2.addrpart in ('03') then gg2.longname
           when gg3.addrpart in ('03') then gg3.longname
           when gg4.addrpart in ('03') then gg4.longname
           when gg5.addrpart in ('03') then gg5.longname
           when gg6.addrpart in ('03') then gg6.longname
           when gg7.addrpart in ('03') then gg7.longname
           when gg8.addrpart in ('03') then gg8.longname
           when gg9.addrpart in ('03') then gg9.longname
           else '#' end as DISTRICT,
      case when gg1.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('03') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as DISTRICT_T,
      case when gg1.addrpart in ('09') then gg1.longname
           when gg2.addrpart in ('09') then gg2.longname
           when gg3.addrpart in ('09') then gg3.longname
           when gg4.addrpart in ('09') then gg4.longname
           when gg5.addrpart in ('09') then gg5.longname
           when gg6.addrpart in ('09') then gg6.longname
           when gg7.addrpart in ('09') then gg7.longname
           when gg8.addrpart in ('09') then gg8.longname
           when gg9.addrpart in ('09') then gg9.longname
           else '#' end CITY,
      case when gg1.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('09') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as CITY_T,
      case when gg1.addrpart in ('04') then gg1.longname
           when gg2.addrpart in ('04') then gg2.longname
           when gg3.addrpart in ('04') then gg3.longname
           when gg4.addrpart in ('04') then gg4.longname
           when gg5.addrpart in ('04') then gg5.longname
           when gg6.addrpart in ('04') then gg6.longname
           when gg7.addrpart in ('04') then gg7.longname
           when gg8.addrpart in ('04') then gg8.longname
           when gg9.addrpart in ('04') then gg9.longname
           else '#' end TOWN,
      case when gg1.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('04') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as TOWN_T,
      case when gg1.addrpart in ('10') then gg1.longname
           when gg2.addrpart in ('10') then gg2.longname
           when gg3.addrpart in ('10') then gg3.longname
           when gg4.addrpart in ('10') then gg4.longname
           when gg5.addrpart in ('10') then gg5.longname
           when gg6.addrpart in ('10') then gg6.longname
           when gg7.addrpart in ('10') then gg7.longname
           when gg8.addrpart in ('10') then gg8.longname
           when gg9.addrpart in ('10') then gg9.longname
           else '#' end as CITY2,
      case when gg1.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('10') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as CITY2_T,
      case when gg1.addrpart in ('06') then gg1.longname
           when gg2.addrpart in ('06') then gg2.longname
           when gg3.addrpart in ('06') then gg3.longname
           when gg4.addrpart in ('06') then gg4.longname
           when gg5.addrpart in ('06') then gg5.longname
           when gg6.addrpart in ('06') then gg6.longname
           when gg7.addrpart in ('06') then gg7.longname
           when gg8.addrpart in ('06') then gg8.longname
           when gg9.addrpart in ('06') then gg9.longname
           else '#' end as MK,
      case when gg1.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('06') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as MT_T,
      case when gg1.addrpart in ('05') then gg1.longname
           when gg2.addrpart in ('05') then gg2.longname
           when gg3.addrpart in ('05') then gg3.longname
           when gg4.addrpart in ('05') then gg4.longname
           when gg5.addrpart in ('05') then gg5.longname
           when gg6.addrpart in ('05') then gg6.longname
           when gg7.addrpart in ('05') then gg7.longname
           when gg8.addrpart in ('05') then gg8.longname
           when gg9.addrpart in ('05') then gg9.longname
           else '#' end as STREET,
      case when gg1.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('05') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as STREET_T,
      case when gg1.addrpart in ('07') then replace(gg1.addrhouse,';','') || decode(gg1.addrbody, null, null, ' корп. ' || gg1.addrbody) ||  decode(gg1.addrbuild, null, null, ' стр. ' || gg1.addrbuild) ||  decode(gg1.addrvlad, null, null, ' влад. ' || gg1.addrvlad)
           when gg2.addrpart in ('07') then replace(gg2.addrhouse,';','') || decode(gg2.addrbody, null, null, ' корп. ' || gg2.addrbody) ||  decode(gg2.addrbuild, null, null, ' стр. ' || gg2.addrbuild) ||  decode(gg2.addrvlad, null, null, ' влад. ' || gg2.addrvlad)
           when gg3.addrpart in ('07') then replace(gg3.addrhouse,';','') || decode(gg3.addrbody, null, null, ' корп. ' || gg3.addrbody) ||  decode(gg3.addrbuild, null, null, ' стр. ' || gg3.addrbuild) ||  decode(gg3.addrvlad, null, null, ' влад. ' || gg3.addrvlad)
           when gg4.addrpart in ('07') then replace(gg4.addrhouse,';','') || decode(gg4.addrbody, null, null, ' корп. ' || gg4.addrbody) ||  decode(gg4.addrbuild, null, null, ' стр. ' || gg4.addrbuild) ||  decode(gg4.addrvlad, null, null, ' влад. ' || gg4.addrvlad)
           when gg5.addrpart in ('07') then replace(gg5.addrhouse,';','') || decode(gg5.addrbody, null, null, ' корп. ' || gg5.addrbody) ||  decode(gg5.addrbuild, null, null, ' стр. ' || gg5.addrbuild) ||  decode(gg5.addrvlad, null, null, ' влад. ' || gg5.addrvlad)
           when gg6.addrpart in ('07') then replace(gg6.addrhouse,';','') || decode(gg6.addrbody, null, null, ' корп. ' || gg6.addrbody) ||  decode(gg6.addrbuild, null, null, ' стр. ' || gg6.addrbuild) ||  decode(gg6.addrvlad, null, null, ' влад. ' || gg6.addrvlad)
           when gg7.addrpart in ('07') then replace(gg7.addrhouse,';','') || decode(gg7.addrbody, null, null, ' корп. ' || gg7.addrbody) ||  decode(gg7.addrbuild, null, null, ' стр. ' || gg7.addrbuild) ||  decode(gg7.addrvlad, null, null, ' влад. ' || gg7.addrvlad)
           when gg8.addrpart in ('07') then replace(gg8.addrhouse,';','') || decode(gg8.addrbody, null, null, ' корп. ' || gg8.addrbody) ||  decode(gg8.addrbuild, null, null, ' стр. ' || gg8.addrbuild) ||  decode(gg8.addrvlad, null, null, ' влад. ' || gg8.addrvlad)
           when gg9.addrpart in ('07') then replace(gg9.addrhouse,';','') || decode(gg9.addrbody, null, null, ' корп. ' || gg9.addrbody) ||  decode(gg9.addrbuild, null, null, ' стр. ' || gg9.addrbuild) ||  decode(gg9.addrvlad, null, null, ' влад. ' || gg9.addrvlad)
           else '#' end as HOUSE,
      case when gg1.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('07') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as HOUSE_T,
      case when gg1.addrpart in ('08') then gg1.longname
           when gg2.addrpart in ('08') then gg2.longname
           when gg3.addrpart in ('08') then gg3.longname
           when gg4.addrpart in ('08') then gg4.longname
           when gg5.addrpart in ('08') then gg5.longname
           when gg6.addrpart in ('08') then gg6.longname
           when gg7.addrpart in ('08') then gg7.longname
           when gg8.addrpart in ('08') then gg8.longname
           when gg9.addrpart in ('08') then gg9.longname
           else '#' end  as  APT,
      case when gg1.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg1.tdc_id)
           when gg2.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg2.tdc_id)
           when gg3.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg3.tdc_id)
           when gg4.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg4.tdc_id)
           when gg5.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg5.tdc_id)
           when gg6.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg6.tdc_id)
           when gg7.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg7.tdc_id)
           when gg8.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg8.tdc_id)
           when gg9.addrpart in ('08') then (select td.shortname from col_G_TERRDSC td where td.id = gg9.tdc_id)
           else '#' end as APT_T
      from col_g_cliaddress g,
           col_g_address    gg1,
           col_g_address    gg2,
           col_g_address    gg3,
           col_g_address    gg4,
    ...
    where 1=1
       and gg1.id = g.adr_id
       and gg2.id(+) = gg1.id_hi
       and gg3.id(+) = gg2.id_hi
       and gg4.id(+) = gg3.id_hi
       and gg5.id(+) = gg4.id_hi
       and gg6.id(+) = gg5.id_hi
       and gg7.id(+) = gg6.id_hi
       and gg8.id(+) = gg7.id_hi
       and gg9.id(+) = gg8.id_hi
       and g.arcfl = 0
       and g.adrtype in ('003','002')
    ;
    commit;
    Update_ETL_Action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100,null);
exception
...
procedure Prepare_etl_bal_loan_all
  is
 l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_bal_loan_all');
 l_action etl_actions_log.id%type;
BEGIN
if check_sysdate_state (l_act_name) is null then
BEGIN
  ...
  truncate_table('etl_bal_loan_all');
  insert /*+ parallel(16) append*/
  into etl_bal_loan_all
    select t.*
      from col_t_bal t
     where t.fromdate < add_months(add_months(trunc(sysdate, 'mm'), -1), 1)
       and t.fromdate >= add_months(trunc(sysdate, 'mm'), -1);

  truncate_table('etl_dm_loan_acc');
  insert
  into etl_dm_loan_acc
    select l.id contract_id,
         l.dep_id,
         acc.acc_id,
         acc.accdep_id,
  ...
  on l.id = d.id
    and d.dep_id = l.dep_id
  inner join col_q_dea qd
  on qd.refer=d.refer
    inner join
         (select qa.obj_id, qa.acc_id, qa.accdep_id, typ.code ,typ.longname,qa.code acc_code
            from col_q_acc qa, col_Q_ENTACC_std typ
           where typ.id = qa.entacc_id
             and sysdate between qa.fromdate and qa.todate) acc
   on acc.obj_id = qd.id
  where acc.code<>'CR_EXP_IN';

  delete from etl_dm_loan_acc a
   where (acc_id, acc_type_code) in
         (select acc_id, max(acc_type_code) acc_type_code
            from etl_dm_loan_acc
           where ACC_TYPE_CODE not in ('КФ04_П', 'CR_EXP_IN')
           group by acc_id
          having count(*) > 1);
  ...
  --DBMS_STATS.GATHER_TABLE_STATS(ownname=>'COLVIR',tabname=>'ETL_DM_LOAN_ACC',cascade=>true);

  truncate_table('etl_bal_loan');
  insert /*+ append*/
  into etl_bal_loan
  select /*--use_hash(t,a) parallel(t,4)*/
    t.*
    from col_t_bal t
  inner join etl_dm_loan_acc a
  on t.id=a.acc_id
  where t.fromdate>=add_months(trunc(sysdate,'mm'),-1);
...
procedure Prepare_etl_tmp_dep_base_2
  is
 l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_tmp_dep_base_2');
 l_action etl_actions_log.id%type;
begin
if check_sysdate_state (l_act_name) is null then
BEGIN
  ...
  Truncate_Table('etl_dep_base_2');

  insert /*+ parallel(16) append*/
  into etl_dep_base_2
    select case
             when t.first_fromdate > sysdate - 10 then
              (select col_t_pkgarl.fPcnValFix(t.dep_id, t.id, t.clc_id, sysdate, 1, 0)
                 from dual) --Maybe Cached?
             else
              cast(null as number)
           end as PRC,
           t.*
     from (select
             (select
               max(o.clc_id)
                from col_T_ARLDEA o, col_T_ARLCLC c, col_T_ARLDSC_STD a
               where o.CLC_ID = c.ID
                 and c.ARL_ID + 0 = a.ID
                 and o.DEP_ID = d.dep_id
                 and o.ORD_ID = d.id
                 and a.CODE in ('DEP_BONUS', 'DEP_BONUS_MM')) CLC_ID,
             m.ID prc_id,
             m.bop_id,
             ...
             d.acc_id ACC_ID,
             d.acc_dep_id ACC_DEP_ID,
             d.first_fromdate FIRST_FROMDATE,
             d.fromdate FROMDATE,
             t.todate TODATE,
             dc.CODE PRD_CODE,
             dc.LONGNAME PRD_NAME,
             ...
             dp.TIME_TYPE TERM_TYPE,
             dp.TIME_UNIT TERM_CNT,
             (select code from col_T_VAL_STD where id = nvl(o.val_id, 1)) VAL_CODE,
             d.prolong CNT_PROLONG,
             s.name STATE,
             s.finishfl STATE_CAT,
             fil.code FIL_CODE,
             fil.longname FIL_NAME,
             substr(col_to_money(t.SDOK, col_t_PkgVal.fGetFac(o.VAL_ID)),1,27) as AMOUNT,
             nvl(col_T_PkgDeaPrm.fPar( t.id, t.dep_id, pp.ID), col_T_PkgDeaPrm.fClsPar(r.dea_id, pp.ID)) NOPROLONG,
             t.sell_dep_id
           from col_D_DEA          d,
                   col_T_Dea          t,
                   col_t_ord          o,
             ...
             where 1 = 1
               ...
               and d.prd_id = dp.ID
               and fil.id = d.dep_id
                  --   and d.fromdate < to_date('01012014','DDMMYYYY')
               and t.todate > to_date('31122009', 'DDMMYYYY')
               ...
               and o.dep_id = d.dep_id
               and o.id = d.id
               and d.acc_id is not null

               and pp.id = r.PAR_ID and r.HIDEFL = 0
               and t.dcl_id = r.dea_id
               and t.dep_id = d.dep_id
               and t.id = d.id
               and dd.DELPHITYP = 'B' and dd.CODE = pp.D_CODE
               and pp.code = 'D_PROLARCFL'
               ) t
   ;
   commit;
   Update_ETL_Action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100,null);
...
procedure Prepare_etl_dm_loan_base_2
  is
 l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_dm_loan_base_2');
 l_action etl_actions_log.id%type;
begin
if check_sysdate_state (l_act_name) is null then
BEGIN
  ...
  store_ETL_ACTion(l_act_name,l_action);
   ...
   Truncate_Table('etl_dm_loan_base_2');

  insert /*+ parallel(16) */
  into etl_dm_loan_base_2
  select /*+use_hash(l,d,dc,com) parallel(l,4) parallel(t,4)*/
   d.id ID,
   d.dep_id DEP_ID,
   dc.id dcl_id,
   col_G_PKGCON.fGetLName(350102, dc.ent_id, l.dep_id, l.id, 0, 'C') product_code_atf,
   col_G_PKGCON.fGetLName(350102, dc.ent_id, l.dep_id, l.id, 0, 'L') product_name_atf,
   col_G_PKGCON.fGetLName(365931, dc.ent_id, l.dep_id, l.id, 0, 'C') target_code_atf,
   col_G_PKGCON.fGetLName(365931, dc.ent_id, l.dep_id, l.id, 0, 'L') target_name_atf,
   col_T_PkgArl.fPcnVal(l.dep_id, l.id, com.pcn_id_c, trunc(sysdate) - 1, 1, 0) pcn_com,
   col_T_PkgArl.fPcnVal(l.dep_id, l.id, com.pcn_id_r, trunc(sysdate) - 1, 1, 0) pcn_rate,
   col_G_PKGCON.fGetLName(350219, dc.ent_id, l.dep_id, l.id, 0, 'C') restruct_code_atf,
   col_G_PKGCON.fGetLName(350219, dc.ent_id, l.dep_id, l.id, 0, 'L') restruct_name_atf
   -- Комиссия за обслуживание займа
   ,col_T_PkgArl.fPcnVal(l.dep_id, l.id, com.PCN_ID_s, trunc(sysdate) - 1, 1, 0) pcn_service
   ,nvl(col_T_PkgDeaPrm.fPar(d.id, d.dep_id, 5106), col_T_PkgDeaPrm.fClsPar(dc.id, 5106)) Effective_rate
    from col_l_dea l
   inner join col_t_dea d
      on l.id = d.id
     and d.dep_id = l.dep_id
    left join col_T_DEACLS_std dc
      on dc.id = d.dcl_id
    left join (select /*+use_hash(o,c)*/
                o.ORD_ID,
                o.DEP_ID,
                max(case
                      when c.ARL_ID = 1681 then
                       c.PCN_ID
                    end) PCN_ID_c,
                max(case
                      when c.ARL_ID = 586 then
                       c.PCN_ID
                    end) PCN_ID_r
                -- Комиссия за обслуживание займа
                ,max(case
                      when c.ARL_ID = 961 then
                       c.PCN_ID
                    end) PCN_ID_s
                ...
                inner join col_T_ARLCLC c
                   on o.CLC_ID = c.ID
                where 1 = 1
                  and c.ARL_ID in (1681, 586, 961)
                group by o.ORD_ID, o.DEP_ID) com
      on com.ord_id = l.id
     and com.dep_id = l.dep_id
   where l.id >= 324171510
     and d.id >= 324171510
  ;
   commit;
   Update_ETL_Action(l_action,gc_Completed,(dbms_utility.get_time-gv_start)/100,null);
...
procedure Prepare_etl_insurance_buf
  is
 l_act_name constant etl_actions_log.action_name%type:=Upper('Prepare_etl_insurance_buf');
 l_action etl_actions_log.id%type;
begin
if check_sysdate_state (l_act_name) is null then
BEGIN
...
/*  Truncate_Table('etl_insurance_buf');

  insert into etl_insurance_buf
  select contract_id,
        max(cvalue) grandes_premium,
        min(cvalue) apolis_premium
  from (
  select \*+use_nl(a,b)*\
        ord_id contract_id,
         to_number(replace(cvalue,'.',',')) cvalue
  from t_operjrn a
  inner join t_operdet b
  on a.id=b.id
    and a.njrn=b.njrn
  where 1=1
      and a.noper=402
      and b.code='AMOUNT'
      and a.doper>=to_date('01.12.2014','dd.mm.yyyy')
  )
...
/*  FILL_COL_L_DEA;
  ...
  /*FILL_COL_T_PROCESS;
  if gv_err_count>0 then goto m; end if;*/
  FILL_COL_T_BOP_STAT_STD;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_T_DEAPRD_std;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_T_DEACLS_std;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_c_dep;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_Q_DEA;
  ...
  FILL_COL_LEDACC_STD;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_G_ACCBLNHST;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_G_ACCBLN;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_G_IDENTDOCDSC_STD;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_T_REG_STD;
  if gv_err_count>0 then goto m; end if;
  FILL_COL_G_CLIHST;
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          and (t.action_name = 'FILL_T_BAL' or t.action_name like 'FILL_COL%')
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=45 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;      
  Prepare_etl_tmp_dep_base_2;
  if gv_err_count>0 then goto m; end if;
  threads_3_4;
  if gv_err_count>0 then goto m; end if;  
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          ...
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=4 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;    
  PREPARE_ETL_DM_CIF_ADDR_TMP;
  if gv_err_count>0 then goto m; end if;  
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          and t.action_name in (
               'FILL_COL_T_DEASHDPNT'
              ,'FILL_COL_T_DEAPAY'
              ,'PREPARE_ETL_BUF'
          )
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=3 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;  
  Prepare_etl_dm_loan_paym_3;
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          ...
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=16 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;  
    
  Prepare_etl_dm_loan_base;
  if gv_err_count>0 then goto m; end if;
  Prepare_etl_CS_CLI_RM_VCC;
  if gv_err_count>0 then goto m; end if;  
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          ...
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=8 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;    
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          ...
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=6 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;  
...
--  if gv_err_count>0 then goto m; end if;
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          ...
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=1 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;   
...
*/  --ETL_PCK_NEW.Fill_LOYAL_New;
  ...
  loop
    ...
    where trunc(t.updated) = trunc(sysdate) 
          ...
          and t.state = 'COMPLETED'; 
    exit when cnt_rec >=14 or count_iter1>=24;
    DBMS_LOCK.Sleep(600);
    count_iter1:=count_iter1+1;
  end loop;  
  if count_iter1>=24 then begin gv_err_count:=gv_err_count+1; goto m; end; end if;  
