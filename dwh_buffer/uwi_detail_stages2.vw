create or replace force view dwh_buffer.uwi_detail_stages2 as
select
rr.REJECT_REASON,
...
case when title like '%Автокредитование%'
                    or  title='Тулпар'
            then 'Авто'
            when title like '%Ипотека%'
            then 'Ипотека'
            when title='Потребительское кредитование'
                or lower(title)='%залог%'
            then 'Под залог'
            when title like '%Легкий%'
            then 'Легкий'
            when title like '%Револьверное кредитование%'
            then 'Револьверное кредитование'
            else title
            end product_group,
...
to_number
 ...
 case
   when
   upper(request_state_code) = upper('Finished') or
                                 --PR_STATUS = 'Завершено'
                                 upper(PR_STATUS) in (upper('Завершено'),
...
upper('Завершено')
 ...
 case
   when row_number() over(partition by t.iin,trunc(t.START_DATE) order by t.START_DATE) = 1 then
    1
   else
    0
 end unique_client_day,
 t.PR_STATUS,
 t.f2_date,
...
case when
t.start_date-nvl(lag(t.start_date) over (partition by t.iin order by t.start_date), to_date('01.01.1999','dd.mm.yyyy'))<30 then 0 else 1 end
no_apps_last_month,
case when row_number() over (partition by t.iin order by t.start_date desc)=1 then 1 else 0 end is_last_application
--,
--s.pd,
--s.pd_cc,
...
left join dwh_buffer.UWI_proc_stages u on t.pr_status=u.status
...
join client c on c.agr_id=t.agr_id
 ...
 where t.dbtype = 2    and
   t.START_DATE >= to_date('2014.05.01', 'yyyy.mm.dd')
  -- and t.client_id=66759

--select * from loan_request t where t.dbtype = 2
--select * from v_uwi_all t  where t.dbtype = 1
;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to AIBEK_BE;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to DWH_PRIM;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to DWH_RISK;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to DWH_SALES;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to HEAD_DENIS_PL;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to HEAD_EVGENIY_PI;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to KRISTINA_KO;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to PROCESS_RISK;
grant select on DWH_BUFFER.UWI_DETAIL_STAGES2 to PROCESS_SALES;
