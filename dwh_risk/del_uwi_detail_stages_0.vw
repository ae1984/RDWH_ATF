create or replace force view dwh_risk.del_uwi_detail_stages_0 as
select
 /*+ parallel(16)*/
 ...
 ,(select lras.registration_number
   from V_LOAN_REQUEST_APPROV_SCOR lras
   where  lras.iin = t.iin
          and trunc(lras.start_date) between trunc(t.finish_date)
...
--t.client_name
 ...
 ,(select lrr.registration_number
   from V_LOAN_REQUEST_REJECT_CANCEL lrr
   where  lrr.iin = t.iin
          and trunc(lrr.finish_date) between trunc(add_months(t.start_date , -1)) and trunc(t.start_date)
          and rownum<2
  ) as lr_REJECT_CANCEL_1
 ,(select lrr.registration_number
   from V_LOAN_REQUEST_REJECT_CANCEL lrr
   where  lrr.iin = t.iin
          and lrr.finish_date between add_months(t.start_date , -2) and trunc(add_months(t.start_date-1,-1))
          and rownum<2
  ) as lr_REJECT_CANCEL_2
 ,(select lrr.registration_number
   from V_LOAN_REQUEST_REJECT_CANCEL lrr
   where  lrr.iin = t.iin
          and lrr.finish_date between add_months(t.start_date , -3) and trunc(add_months(t.start_date-1,-2))
          and rownum<2
...
from dwh_buffer.loan_request t
  ...
  left join V_LOAN_REQUEST_REJECT_CANCEL lrrc on lrrc.registration_number = t.registration_number
  left join V_LOAN_REQUEST_APPROV_SCOR lra on lra.registration_number = t.registration_number
