create or replace force view dwh_risk.v_loanreq_cp_20160115_1 as
select /*+parallel(32)*/
   ...
   ,case when trunc(t.csln_fromdate,'MM') = trunc(t.report_dt,'MM') then 1 else 0 end as is_issue_mm --подмена дат что меньше 29/12/2012 на 29/12/2012
...
from (
      select  aa.refer_wh
             ,aa.sell_dep_id
             ,case when nvl(aa.csln_fromdate,aa.fromdate) <= to_date('29.12.2012') then to_date('29.12.2012') else nvl(aa.csln_fromdate,aa.fromdate) end as csln_fromdate
             ,bb.*
      ...
      where upper(bb.f77) in ('CORP','SNT')
     )t
 /*right*/ left join (
