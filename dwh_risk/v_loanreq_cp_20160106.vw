create or replace force view dwh_risk.v_loanreq_cp_20160106 as
select /*+parallel(32)*/
   t.report_dt report_dt1
   ,trunc(t.report_dt,'mm') report_dt,
   --trunc(t.report_dt,'q') report_Q
   case
      when extract (month from trunc(t.report_dt,'q')) = 1 then 'QI'
      when extract (month from trunc(t.report_dt,'q')) = 4 then 'QII'
      when extract (month from trunc(t.report_dt,'q')) = 7 then 'QIII'
      when extract (month from trunc(t.report_dt,'q')) = 10 then 'QIV'
   end as report_Q
   ...
   ,case when nvl(lead(t.F82,1) over (partition by t.F53 order by t.report_dt),0) > 0 then 1 else 0 end as FPD0
   ,case when nvl(lead(t.F82,3) over (partition by t.F53 order by t.report_dt),0) > 30 then 1 else 0 end as FPD60
   ,case when nvl(lead(t.F82,2) over (partition by t.F53 order by t.report_dt),0) > 30 then 1 else 0 end as is_OVRD_lead2M_30p
   ,case when nvl(lead(t.F82,3) over (partition by t.F53 order by t.report_dt),0) > 30 then 1 else 0 end as is_OVRD_lead3M_30p
   ,case when nvl(lead(t.F82,6) over (partition by t.F53 order by t.report_dt),0) > 30 then 1 else 0 end as is_OVRD_lead6M_30p
   ,case when nvl(lead(t.F82,12) over (partition by t.F53 order by t.report_dt),0) > 90 then 1 else 0 end as is_OVRD_lead12M_90p
   ...
   --,t.f51 as is_RWO
   ,case
      when trunc(t.csln_fromdate,'MM') = trunc(t.report_dt,'MM') and upper(nvl(t.f35,'-')) in ('N','-') then 'Ќовый, не реструктурированный'
      when trunc(t.csln_fromdate,'MM') = trunc(t.report_dt,'MM') and upper(nvl(t.f35,'-')) not in ('N','-') then 'Ќовый, отструктурированный'
      when trunc(t.csln_fromdate,'MM') <> trunc(t.report_dt,'MM') and upper(nvl(t.f35,'-')) in ('N','-') then 'Ќе новый, не реструктурированный'
      when trunc(t.csln_fromdate,'MM') <> trunc(t.report_dt,'MM') and upper(nvl(t.f35,'-')) not in ('N','-') then 'Ќе новый, реструктурированный'
      else 'неопределен (возможно нет даты открыти€)'
    end as is_flag_stat
   ,t.f11 as t_loan_sum
   ...
   ,case when trunc(a.income/25000)*25000>600000 then 600000 else trunc(a.income/25000)*25000 end as income_group
   ,b.product_type
   ,t.f35
   ,t.csln_fromdate
   ,t.f6
