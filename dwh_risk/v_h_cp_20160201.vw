create or replace force view dwh_risk.v_h_cp_20160201 as
select
  t.report_dt,
  t.F53 REFER,
  t.f9 CONTRACT_NUM
...
where --t.f53 is not null and nvl(trim(t.f1),'-') <> 'МФО' and nvl(upper(trim(t.f2)),'-') <>upper('Лицевой счет') and
      t.report_dt >= add_months(trunc(sysdate,'mm'),-18)
;
